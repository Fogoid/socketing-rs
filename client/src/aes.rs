...
impl AESAlgorithm {
    pub fn new_aes128() -> AESAlgorithm {
        AESAlgorithm::AES128(AESParams {
            n_k: 4,
            n_b: 4,
            n_r: 10,
        })
    ...
    pub fn generate_key(&self) -> [u8; 16] {
        match self {
            AESAlgorithm::AES128(_) => generate_key(),
        }
    }

    pub fn key_expansion(&self, key: &[u8; 16]) -> [[u8; 16]; 11] {
        let params = match self {
            AESAlgorithm::AES128(params) => params,
        };
        let state = convert_to_state(&key);
        let mut keys = [[0u8; 16]; 11];
        keys[0] = *key;

        for r in 0..params.n_r {
            ...
            let key = convert_from_state(&state);
            keys[r+1] = key;
        }
...
pub fn generate_key() -> [u8; 16] {
    ...
    key.try_fill(&mut rand::thread_rng())
        .expect("Expected vector to be filled with random values");
...
fn sub_word(word: &mut [u8; 4]) {
    for i in 0..4 {
        let x = (word[i] >> 4) as usize;
        let y = (word[i] & 0x0F) as usize;
        word[i] = S_BOX[x][y];
    }
}

fn convert_to_state(data: &[u8; 16]) -> [[u8; 4]; 4] {
...
fn convert_from_state(state: &[[u8; 4]; 4]) -> [u8; 16] {
    let mut data = [0u8; 16];
